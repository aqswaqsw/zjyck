...
function sign_in(access_token, remarks) {
    ...
    .then(async json => {
        ...
        const currentSignInfo = signInLogs[signInCount - 1];
        ...
        if (rewards.length) {
            for await (reward of rewards) {
                ...
                try {
                    ...
                    sendMessage.push(`第${signInDay}天奖励领取成功: 获得${rewardInfo.name || ''}${rewardInfo.description || ''}`);
                } catch (e) {
                    sendMessage.push(`第${signInDay}天奖励领取失败:`, e);
                }
            }
        } else if (currentSignInfo.isReward) {
            sendMessage.push(`今日签到获得${currentSignInfo.reward.name || ''}${currentSignInfo.reward.description || ''}`);
        }
...
async function getRefreshToken() {
    ...
    return { instance, refreshTokenArray };
}
        ...
        try {
            ...
            if (instance) {
                let params = {
                    ...
                    value: refresh_token,
                    remarks: refreshToken.remarks || nick_name
                };
                if (refreshToken.id) params.id = refreshToken.id;
                if (refreshToken._id) params._id = refreshToken._id;
